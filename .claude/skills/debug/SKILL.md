...
# OPENCODE_ZEN_API_KEY=sk-zen-...           (pay-per-use)
```
...
**Workaround:** The system extracts only authentication variables (`CLAUDE_CODE_OAUTH_TOKEN`, `OPENCODE_ZEN_API_KEY`) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed.
...
echo '{}' | docker run -i \
  ...
  -c 'export $(cat /workspace/env-dir/env | xargs); echo "OAuth: ${#CLAUDE_CODE_OAUTH_TOKEN} chars, API: ${#OPENCODE_ZEN_API_KEY} chars"'
```
...
```
/workspace/
├── env-dir/env           # Environment file (CLAUDE_CODE_OAUTH_TOKEN or OPENCODE_ZEN_API_KEY)
...
[ -f .env ] && (grep -q "CLAUDE_CODE_OAUTH_TOKEN=sk-" .env || grep -q "OPENCODE_ZEN_API_KEY=sk-" .env) && echo "OK" || echo "MISSING - add CLAUDE_CODE_OAUTH_TOKEN or OPENCODE_ZEN_API_KEY to .env"
